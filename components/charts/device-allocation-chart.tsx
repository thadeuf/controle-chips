...
export function DeviceAllocationChart({ data, title, color = "#6366f1" }: DeviceAllocationChartProps) {
  ...
  useEffect(() => {
    ...
    data.forEach((point, index) => {
      ...
      // Desenhar valor
      ctx.textAlign = "left"
      ctx.fillText(`${point.value} números`, padding + barWidth + 5, y + barHeight / 2 + 4)
    })
    ...
    ctx.textAlign = "center"
    ctx.font = "12px Arial"
    ctx.fillText("Quantidade", canvas.width / 2, canvas.height - 15)
  }, [data, title, color])
